...
<div class="cell medium-6">
	<div class="grid-y align-left" style="height: 100%">
		...
		add_filter ( 'woocommerce_product_loop_start', function($loopStart) {
			...
			if (is_front_page()) {
				$loopStart = "<ul class='products grid-x small-up-2 align-center'>";
			}
			else {
				$loopStart = "<ul class='products grid-x small-up-1 medium-up-3 align-center'>";
			}
			return $loopStart;
		...
		add_filter('post_class', function($classes, $class, $product_id) {
			...
			if( is_product_category() || is_shop() || (is_front_page() && 'product' == get_post_type())) {
...
function austeve_myaccount_before_nav() {
	?>
	<div class="grid-x grid-margin-x">
		...
		function austeve_myaccount_after_nav() {
			?>
		</div>
		<?php
	}

	function austeve_myaccount_before_panel() {
		?>
		<div class="cell medium-9">
			<?php
		}

		function austeve_myaccount_after_panel() {
			?>
		</div>
	</div>
	<?php
}
add_action ( 'woocommerce_before_account_navigation', 'austeve_myaccount_before_nav', 9);
add_action ( 'woocommerce_after_account_navigation', 'austeve_myaccount_after_nav', 11 );
add_action ( 'woocommerce_after_account_navigation', 'austeve_myaccount_before_panel', 12 );
add_action ( 'woocommerce_account_content', 'austeve_myaccount_after_panel', 20 );
